...
[[nodiscard]] core::ErrorCode MakeErrnoError(std::string message) {
  return {"someip-udp", std::move(message) + ": " + std::strerror(errno)};
}
...
[[nodiscard]] core::Result<sockaddr_in> ToSockaddr(const UdpEndpointAddress& address) {
  ...
  if (::inet_pton(AF_INET, address.host.c_str(), &socket_address.sin_addr) != 1) {
    return core::Result<sockaddr_in>::FromError({"someip-udp", "invalid IPv4 address"});
  }
...
core::Result<UdpEndpointAddress> UdpEndpoint::LocalAddress() const {
  if (fd_ < 0) {
    return core::Result<UdpEndpointAddress>::FromError({"someip-udp", "endpoint is closed"});
  }

  sockaddr_in address{};
  socklen_t length = sizeof(address);
  if (::getsockname(fd_, reinterpret_cast<sockaddr*>(&address), &length) < 0) {
    return core::Result<UdpEndpointAddress>::FromError(MakeErrnoError("failed to read local address"));
  }
...
core::Result<std::size_t> UdpEndpoint::SendTo(
  ...
  if (fd_ < 0) {
    return core::Result<std::size_t>::FromError({"someip-udp", "endpoint is closed"});
  }
...
core::Result<UdpDatagram> UdpEndpoint::Receive() const {
  if (fd_ < 0) {
    return core::Result<UdpDatagram>::FromError({"someip-udp", "endpoint is closed"});
  }
  ...
  auto message = DeserializeMessage(bytes);
  if (!message) {
